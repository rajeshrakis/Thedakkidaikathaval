...
from Thedakkidaikkathaval import DRAGONS
from Thedakkidaikkathaval.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
...
async def rakis_is_admin(chat_id: int):
    status = False
    rakis = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if rakis.id == user.id:
